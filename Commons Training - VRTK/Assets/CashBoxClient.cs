...
public class CashBoxClient : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (client.askingQuestion && !questionAsked && timer <= 0)
        {
            ...
            randomIndex = Random.Range(0, QuestionInput.cashBoxCorrectAnswers.Count);
            questions.GetComponentInChildren<TextMeshProUGUI>().text = "Excuse me, " + QuestionInput.cashBoxQuestions[randomIndex];
            output = new List<string> { };

            output.Add(QuestionInput.cashBoxCorrectAnswers[randomIndex]);
            output.Add(QuestionInput.cashBoxWrongAnswers[randomIndex][0]);
            output.Add(QuestionInput.cashBoxWrongAnswers[randomIndex][1]);
            output.Add(QuestionInput.cashBoxWrongAnswers[randomIndex][2]);
            ...
            correctAnswer = QuestionInput.cashBoxCorrectAnswers[randomIndex];
        ...
        if (questionAnswered && questionAsked)
        {
            ...
            QuestionInput.cashBoxQuestions.RemoveAt(randomIndex);
            QuestionInput.cashBoxCorrectAnswers.RemoveAt(randomIndex);
            QuestionInput.cashBoxWrongAnswers.RemoveAt(randomIndex);
            answers.SetActive(false);
            questionAsked = false;
